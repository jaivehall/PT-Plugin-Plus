  ...
  <div class="userDataTimeline">
    ...
    <div class="toolbar">
      ...
      <v-switch
        ...
        class="my-0"
      ></v-switch>
      <v-text-field class="search" v-model="txtWatermark" :label="$t('timeline.watermark')"
          @input="setWatermark()"></v-text-field>
      <v-divider />
...
export default Vue.extend({
  ...
  mounted() {
    this.txtWatermark = "仅限 "+(this.displayUserName ||this.infos.nameInfo.name)+" 个人使用"
    this.setWatermark()
  },
  watch:{
    'infos.nameInfo.name'( v ){
      this.txtWatermark = v ? "仅限 "+v+" 个人使用": v
      this.setWatermark()
    }
  },
  computed: {
  ...
  methods: {
    setWatermark(){
      Watermark.set(this.txtWatermark, this.$refs.userDataCard)
    },
